...
app.controller("UserSelectionCtrl", function ($scope, EventFactory, $q) {
  ...
  let eventArrays = [preferencesCoffee, preferencesBreweries, preferencesLiveMusic, preferencesTours, preferencesRestaurants, preferencesCityPark, preferencesWaterfallHikes, preferencesShopping];
  let eventTypes = ["coffee", "breweries", "liveMusic", "tours", "restaurants", "cityPark", "waterfallHikes", "shopping"];
  let eventTypesForDom = ["Coffee Shops", "Breweries", "Live Music", "Tours", "Restaurants", "City Parks", "Waterfall Hikes", "Shopping"];
  let eventObject = [];
  ...
  function getAllPreferredEvents() {
    ...
    $q.all(promises).then( (data) => {
      flattenObjects2(data);
    });
  }


  function flattenObjects2(data){
      console.log("inside flattenobject (new) data", data)
    data.forEach(function(object){
      for(var singleObject in object){
        for(var i = 0; i < data.length; i++){
          if(object[singleObject].type === eventTypes[i]){
            console.log("object type", object[singleObject].type)
            console.log("event array to push to", eventTypes[i])
            eventArrays[i].push(object[singleObject])
          }
        }
      }
    })
  ...
  //   data.forEach(function(object) {
  //   console.log("object in flatten object", object);
  ...
  function buildObjectsForDom2(){
    $scope.objectsForDOM = [];

    for(var i = 0; i < eventArrays.length; i++){
      if(eventArrays[i].length >= 1) {
        eventObject[i] = {
          eventType: eventTypesForDom[i],
          events: eventArrays[i]
        };
        console.log(eventObject[i])
        $scope.objectsForDOM.push(eventObject[i]);
      }
    }
  }
